SELECT
    ...
    greatest(
        1 - percentile_disc(0.25) WITHIN GROUP (ORDER BY COALESCE((node->'metrics_latency')::float, 100.)) / 2.5,
        0
    ) as metrics_latency_score_p25,

    greatest(
        1 - percentile_disc(0.95) WITHIN GROUP (ORDER BY COALESCE((node->'metrics_latency')::float, 100.)) / 2.5,
        0
    ) as metrics_latency_score_p95,

    greatest(
        1 - percentile_disc(0.25) WITHIN GROUP (ORDER BY COALESCE((node->'aggregate_latency')::float, 100.)) / 4,
        0
    ) as aggregate_latency_score_p25,

    greatest(
        1 - percentile_disc(0.95) WITHIN GROUP (ORDER BY COALESCE((node->'aggregate_latency')::float, 100.)) / 4,
        0
    ) as aggregate_latency_score_p95,

    greatest(
        1 - percentile_disc(0.25) WITHIN GROUP (ORDER BY COALESCE((node->'file_download_latency')::float, 100.)) / 4,
        0
    ) as file_download_latency_score_p25,

    greatest(
        1 - percentile_disc(0.95) WITHIN GROUP (ORDER BY COALESCE((node->'file_download_latency')::float, 100.)) / 4,
        0
...
WHERE
    owner = ANY(ARRAY['0x4d741d44348B21e97000A8C9f07Ee34110F7916F', '0x4d741d44348B21e97000A8C9f07Ee34110F7916F'])
